...
//        RedisUtil.INSTANCE.sadd(REDIS_SET_ACTIVE_SUBJECTS, subject);
    ...
    public static String myParseToken(HttpServletRequest request, String tokenName, String signingKey){
        String token = request.getHeader(tokenName);
        String username = Jwts.parser().setSigningKey(signingKey).parseClaimsJws(token).getBody().getSubject();
        return username;
    }
